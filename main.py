...
class ChatPage(ttk.Frame):
    def __init__(self, parent, controller):
        ...
        thread.Thread(target=self.listen_to_server).start()
    ...
    def send_message(self):
        ...
        if message and self.current_user:
            ...
            self.client.send(jh.json_encode("room_message", {"room": self.current_user, "message": message}))
            self.update_chat_history()
            self.message_entry.delete(0, tk.END)

    def listen_to_server(self):
        while True:
            received = self.client.ssl_clientsocket.recv(1024).decode()
            if received:
                data = jh.json_decode(received)
                room = data.get("room")
                message = data.get("message")
                if room and message:
                    if room not in self.chat_histories:
                        self.chat_histories[room] = []
                    self.chat_histories[room].append(message)
                    if room == self.current_user:
                        self.update_chat_history()
